...
class AdminController extends Controller
{
    ...
    public function guardarUsuario(Request $request){
        ...
        Mail::send('mailslayouts.nuevousuario', $data, function($message) use($mail)
            {
                $message->to($mail, 'Nuevo Usuario')->subject('Usuario creado')->from('no-reply@urgenciasmedicas.net', 'Urgencias Médicas');
            });
    ...
    public function guardarFuneraria(Request $request){
        ...
        Mail::send('mailslayouts.nuevaFuneraria', $data, function($message) use($mail)
            {
                $message->to($mail, 'Nueva funeraria')->subject('Usuario creado')->from('no-reply@urgenciasmedicas.net', 'Urgencias Médicas');
            });
    ...
    public function guardarCambiosFuneraria($id, Request $request){
        ...
        if($old_Activa != $request->activo){
            if($activa == 'Activo'){
                ...
                {
                    $message->to($mail, 'Funeraria')->subject('Su usuario ha sido activado')->from('no-reply@urgenciasmedicas.net', 'Urgencias Médicas');
                });
                ...
                {
                    $message->to($mail, 'Funeraria')->subject('Su usuario ha sido desactivado')->from('no-reply@urgenciasmedicas.net', 'Urgencias Médicas');
                });
