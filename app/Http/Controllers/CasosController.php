...
class CasosController extends Controller
{
    ...
    public function asignarFuneraria($caso, $funeraria, $campania, $moneda, $correo, $wp){
        ...
        //if($correo == 'Si'){
            ...
            {
                $message->to($correo_funeraria, $nombre_funeraria)->subject($mensaje)->from('no-reply@mail.centrourgenciasmedicas.com', 'Urgencias Médicas');
            });
        //}
        /*if($wp == 'Si'){
    ...
    public function actualizarPago($caso, Request $request){
        ...
        for ($i=1; $i <= $request->filas ; $i++) { 
            ...
            if(isset($correo_funeraria)){
                ...
                {
                    $message->to($correo_admin, $nombre_funeraria)->subject($mensaje)->from('no-reply@mail.centrourgenciasmedicas.com', 'Urgencias Médicas');
                });
            }
    ...
    public function cerrarCaso($caso){
        ...
        Mail::send('mailslayouts.encuesta', $data, function($message)
            {
                $message->to('samuelambrosio99@gmail.com', 'test')->subject('Encuesta UMFunerarias')->from('no-reply@mail.centrourgenciasmedicas.com', 'Urgencias Médicas');
            });
